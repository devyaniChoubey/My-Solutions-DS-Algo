...
class Solution {
public:
    int noOfStallsPossible(vector<int> &stalls, int minDist){
        int stallsCnt = 1;
        int prevStallsPos = stalls[0];
        
        for(int i=1;i < stalls.size();i++){
            
            if(stalls[i] >= prevStallsPos + minDist){
                stallsCnt++;
                prevStallsPos = stalls[i];
            }
        }
    ...
    int solve(int n, int k, vector<int> &stalls) {
        ...
        while(low <= high){
            ...
            if(noOfStallsPossible(stalls, mid) >= k){
                low = mid + 1;
            }else{
                high = mid - 1;
            }
